...
export default function UserPublicPage() {
  ...
  const [sortBy, setSortBy] = useState('latest'); // default: newest first (matches admin panel)
    ...
    const loadUserData = async () => {
      if (userId === 'guest') {
        try {
          ...
          console.error('Error reading localStorage for guest', e);
        }
        
        // Fallback to global links if localStorage is empty
        try {
      ...
      <main className="bio-container">
        ...
        <header className="profile-card glass-panel">
          ...
          <div className="profile-name-row">
            ...
            <Link href="/admin" className="admin-lock-btn" title="관리자 페이지">
              <Lock size={14} />
              <span>관리</span>
            </Link>
          </div>
